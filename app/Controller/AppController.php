...
$homepages5 = $this->Homepage->find('all', array(
	...
	$tags = $this->Category->find('threaded', array(
			'contain' => [
                'ChildCategory',
	...
	$colleges = $this->Category->find('threaded', array(
			'contain' => [
                'ChildCategory',
			...
			'conditions' => array('Category.parent_id' => '39',
			'not' => array('Category.name' => null),
			'Category.active' => '1',
		...
		$this->loadModel('Question');
		 $popular = $this->Question->find('all', array(
			'recursive' => 0,
			'contain' => 'Category', 
			'order' => array(
				'Question.views' => 'DESC'
 ...
 $this->loadModel('Complaint');
		$latest_speak = $this->Complaint->find('all', array(
			'recursive' => 0,
			'contain' => 'Category', 
			'order' => array(
				'Complaint.id' => 'DESC',
		 ...
		 $this->loadModel('Question');
		$latest_question = $this->Question->find('all', array(
			'recursive' => 0,
			'contain' => 'Category', 
			'order' => array(
				'Question.id' => 'DESC'
	...
	$answers_stat_count = $this->Answer->find('count', array(
			'recursive' => 0,
			'order' => array(
				'Answer.id' => 'DESC'
			),
        ...
        $this->set(compact('latest_speak','answers_stat_count'));
		
		$this->loadModel('Complaint');
		$speakers_stat_count= $this->Complaint->find('count', array(
			'recursive' => 0,
			'order' => array(
				'Complaint.id' => 'DESC',
        ...
        $this->set(compact('speakers_stat_count'));
		$this->loadModel('Question');
	$questions_stat_count = $this->Question->find('count', array(
			'recursive' => 0,
			'order' => array(
				'Question.id' => 'DESC'
			),
        ...
        $this->set(compact('colleges','tags','popular','questions_stat_count'));
	  $this->loadModel('Blog');	
	...
	$ourblogs = $this->Blog->find('all', array(
        ...
        $this->set(compact('latest_question','ourblogs')); 
...
$categories1 = $this->Category->find('threaded',$params1);
  ...
  $home_categories = $this->Category->find('all', array(
		...
		$this->set(compact('categories','home_categories','footercategories','footercategories1'));
