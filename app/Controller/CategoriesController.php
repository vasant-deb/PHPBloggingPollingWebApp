 ...
 $sizes = "";
  ...
  if(!empty( $this->request->query['orderby'])) { 
   ...
   if(!empty( $this->request->query['pagelimit'])) { 
	...
	$conditions['Complaint.category_id'] = Hash::extract($categoryIds, '{n}.Category.id');
	$this->loadModel('Complaint');	
	$categories_speak = $this->Complaint->find('all', [    
			'contain' => array(
				'Category',
			),	
            'conditions' => [
               'Complaint.category_id' => $category['Category']['id'],
	...
	$this->set(compact('categories_speak','pagelimit'));	
