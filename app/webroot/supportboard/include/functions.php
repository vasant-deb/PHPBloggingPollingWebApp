 ...
 * -----------------------------------------------------------
 *
 * Delete the user (or users) and all related information(conversations, messages)
 *
 */
 ...
 * -----------------------------------------------------------
 *
 * Return the user details of each conversation
 *
 */
 ...
 * -----------------------------------------------------------
 *
 * Check if the current time is in the office hours
 *
 */
 ...
 * -----------------------------------------------------------
 *
 * Create .csv file from the given array
 *
 */
 ...
 * ----------------------------------------------------------
 *
 * Generate the css with the settings values
 *
 */
 ...
 * ----------------------------------------------------------
 *
 * Delete visitors older than 24h and messages in trash older than 30 days
 *
 */
...
function sb_get_versions() {
    return json_decode(file_get_contents('https://board.support/synch/versions.json'), true);
}
...
function sb_app_activation($app_name, $key) {
    $response = json_decode(file_get_contents('https://board.support/synch/updates.php?' . $app_name . '=' . $key), true);
...
function sb_app_update($app_name, $file_name, $key = false) {
    if ($file_name == '') {
        return 'temporary-file-name-not-found';
    }
...
function sb_update() {
    ...
    if ($envato_code == '') {
        return 'envato-purchase-code-not-found';
    }
    ...
    $downloads = json_decode(file_get_contents('https://board.support/synch/updates.php?' . substr($link, 0, -1)), true);
